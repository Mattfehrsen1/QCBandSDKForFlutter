    ...
    */
    ...
    public static final int commonReadUtf8CodePoint(Buffer buffer) throws EOFException {
        ...
        if ((b & 128) == 0) {
            ...
            i = b & Constants.CMD_DISPLAY_TIME;
            i2 = 2;
            i3 = 128;
        } else if ((b & Mmi.AU_MMI_START_ROLESWAP) == 224) {
            i = b & 15;
            i2 = 3;
            i3 = 2048;
        } else {
            if ((b & Mmi.AU_MMI_AV_PAUSE) != 240) {
                buffer.skip(1L);
        ...
        if (1 < i2) {
            ...
            while (true) {
                ...
                if ((b2 & Mmi.AU_MMI_SD_PLAYBACK_SWITCH) != 128) {
                    buffer.skip(j2);
                    return Utf8.REPLACEMENT_CODE_POINT;
                }
                i = (i << 6) | (b2 & 63);
    ...
    public static final long commonIndexOf(Buffer buffer, byte b, long j, long j2) {
        ...
        if (!z) {
            throw new IllegalArgumentException(("size=" + buffer.size() + " fromIndex=" + j3 + " toIndex=" + size).toString());
        }
        if (size > buffer.size()) {
            size = buffer.size();
        }
        ...
        if (buffer.size() - j3 < j3) {
            ...
            while (size2 < j5) {
                byte[] bArr = segment.data;
                int iMin = (int) Math.min(segment.limit, (segment.pos + j5) - size2);
                for (int i = (int) ((segment.pos + j3) - size2); i < iMin; i++) {
                    if (bArr[i] == b) {
                        return (i - segment.pos) + size2;
                    }
                }
                size2 += segment.limit - segment.pos;
                segment = segment.next;
        ...
        while (j4 < j5) {
            byte[] bArr2 = segment.data;
            int iMin2 = (int) Math.min(segment.limit, (segment.pos + j5) - j4);
            for (int i2 = (int) ((segment.pos + j3) - j4); i2 < iMin2; i2++) {
                if (bArr2[i2] == b) {
    ...
    public static final long commonIndexOf(Buffer buffer, ByteString bytes, long j) {
        ...
        if (buffer.size() - j < j) {
            ...
            while (j4 < size3) {
                ...
                int iMin = (int) Math.min(segment2.limit, (segment2.pos + size3) - j4);
                int i = (int) ((segment3.pos + j5) - j4);
                if (i < iMin) {
                    while (true) {
                        int i2 = i + 1;
                        if (bArr[i] == b && rangeEquals(segment3, i2, bArrInternalArray$okio, 1, size2)) {
                            return (i - segment3.pos) + j4;
                        }
                        if (i2 >= iMin) {
                            break;
                        }
        ...
        while (j7 < size5) {
            ...
            long j9 = size5;
            int iMin2 = (int) Math.min(segment4.limit, (segment4.pos + size5) - j7);
            int i3 = (int) ((segment4.pos + j8) - j7);
            if (i3 < iMin2) {
                while (true) {
                    int i4 = i3 + 1;
                    if (bArr2[i3] == b2 && rangeEquals(segment4, i4, bArrInternalArray$okio2, 1, size4)) {
