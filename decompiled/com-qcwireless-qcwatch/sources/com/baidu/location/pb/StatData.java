...
public final class StatData extends MessageMicro {
    ...
    public int getSerializedSize() {
        ...
        int iComputeInt32Size = hasLoadSuccCnt() ? CodedOutputStreamMicro.computeInt32Size(1, getLoadSuccCnt()) + 0 : 0;
        if (hasLoadFailCnt()) {
            iComputeInt32Size += CodedOutputStreamMicro.computeInt32Size(2, getLoadFailCnt());
        }
        if (hasLocSuccCnt()) {
            iComputeInt32Size += CodedOutputStreamMicro.computeInt32Size(3, getLocSuccCnt());
        }
        if (hasLocFailCnt()) {
            iComputeInt32Size += CodedOutputStreamMicro.computeInt32Size(4, getLocFailCnt());
        }
        Iterator<Integer> it = getAccsList().iterator();
        int iComputeInt32SizeNoTag2 = 0;
        while (it.hasNext()) {
            iComputeInt32SizeNoTag2 += CodedOutputStreamMicro.computeInt32SizeNoTag(it.next().intValue());
        }
        int size = iComputeInt32Size + iComputeInt32SizeNoTag2 + (getAccsList().size() * 1);
        Iterator<Integer> it2 = getCostsList().iterator();
        while (it2.hasNext()) {
            iComputeInt32SizeNoTag += CodedOutputStreamMicro.computeInt32SizeNoTag(it2.next().intValue());
        }
        int size2 = size + iComputeInt32SizeNoTag + (getCostsList().size() * 1);
