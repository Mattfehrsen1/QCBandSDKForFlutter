package com.qcwireless.qcwatch.ui.base.bean.request.device;

import com.google.android.gms.fitness.FitnessActivities;
import kotlin.Metadata;
import kotlin.jvm.internal.Intrinsics;

/* compiled from: DeviceFeaturesListRequest.kt */
@Metadata(d1 = {"\u0000\"\n\u0002\u0018\u0002\n\u0002\u0010\u0000\n\u0000\n\u0002\u0010\u000e\n\u0002\b\t\n\u0002\u0010\u000b\n\u0002\b\u0002\n\u0002\u0010\b\n\u0002\b\u0002\b\u0086\b\u0018\u00002\u00020\u0001B\u0015\u0012\u0006\u0010\u0002\u001a\u00020\u0003\u0012\u0006\u0010\u0004\u001a\u00020\u0003¢\u0006\u0002\u0010\u0005J\t\u0010\t\u001a\u00020\u0003HÆ\u0003J\t\u0010\n\u001a\u00020\u0003HÆ\u0003J\u001d\u0010\u000b\u001a\u00020\u00002\b\b\u0002\u0010\u0002\u001a\u00020\u00032\b\b\u0002\u0010\u0004\u001a\u00020\u0003HÆ\u0001J\u0013\u0010\f\u001a\u00020\r2\b\u0010\u000e\u001a\u0004\u0018\u00010\u0001HÖ\u0003J\t\u0010\u000f\u001a\u00020\u0010HÖ\u0001J\t\u0010\u0011\u001a\u00020\u0003HÖ\u0001R\u0011\u0010\u0002\u001a\u00020\u0003¢\u0006\b\n\u0000\u001a\u0004\b\u0006\u0010\u0007R\u0011\u0010\u0004\u001a\u00020\u0003¢\u0006\b\n\u0000\u001a\u0004\b\b\u0010\u0007¨\u0006\u0012"}, d2 = {"Lcom/qcwireless/qcwatch/ui/base/bean/request/device/DeviceFeaturesListRequest;", "", "hardwareVersion", "", "romVersion", "(Ljava/lang/String;Ljava/lang/String;)V", "getHardwareVersion", "()Ljava/lang/String;", "getRomVersion", "component1", "component2", "copy", "equals", "", FitnessActivities.OTHER, "hashCode", "", "toString", "app_qwatch_proRelease"}, k = 1, mv = {1, 6, 0}, xi = 48)
/* loaded from: classes3.dex */
public final /* data */ class DeviceFeaturesListRequest {
    private final String hardwareVersion;
    private final String romVersion;

    public static /* synthetic */ DeviceFeaturesListRequest copy$default(DeviceFeaturesListRequest deviceFeaturesListRequest, String str, String str2, int i, Object obj) {
        if ((i & 1) != 0) {
            str = deviceFeaturesListRequest.hardwareVersion;
        }
        if ((i & 2) != 0) {
            str2 = deviceFeaturesListRequest.romVersion;
        }
        return deviceFeaturesListRequest.copy(str, str2);
    }

    /* renamed from: component1, reason: from getter */
    public final String getHardwareVersion() {
        return this.hardwareVersion;
    }

    /* renamed from: component2, reason: from getter */
    public final String getRomVersion() {
        return this.romVersion;
    }

    public final DeviceFeaturesListRequest copy(String hardwareVersion, String romVersion) {
        Intrinsics.checkNotNullParameter(hardwareVersion, "hardwareVersion");
        Intrinsics.checkNotNullParameter(romVersion, "romVersion");
        return new DeviceFeaturesListRequest(hardwareVersion, romVersion);
    }

    public boolean equals(Object other) {
        if (this == other) {
            return true;
        }
        if (!(other instanceof DeviceFeaturesListRequest)) {
            return false;
        }
        DeviceFeaturesListRequest deviceFeaturesListRequest = (DeviceFeaturesListRequest) other;
        return Intrinsics.areEqual(this.hardwareVersion, deviceFeaturesListRequest.hardwareVersion) && Intrinsics.areEqual(this.romVersion, deviceFeaturesListRequest.romVersion);
    }

    public int hashCode() {
        return (this.hardwareVersion.hashCode() * 31) + this.romVersion.hashCode();
    }

    public String toString() {
        return "DeviceFeaturesListRequest(hardwareVersion=" + this.hardwareVersion + ", romVersion=" + this.romVersion + ')';
    }

    public DeviceFeaturesListRequest(String hardwareVersion, String romVersion) {
        Intrinsics.checkNotNullParameter(hardwareVersion, "hardwareVersion");
        Intrinsics.checkNotNullParameter(romVersion, "romVersion");
        this.hardwareVersion = hardwareVersion;
        this.romVersion = romVersion;
    }

    public final String getHardwareVersion() {
        return this.hardwareVersion;
    }

    public final String getRomVersion() {
        return this.romVersion;
    }
}
