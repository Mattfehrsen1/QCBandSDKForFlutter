...
public class QBloodOxygenLineChartView extends BaseChartView {
    ...
    public QBloodOxygenLineChartView(Context context) {
        super(context);
        this.yMax = 90;
    ...
    public QBloodOxygenLineChartView(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.yMax = 90;
        this.timeArray = new int[24];
        this.labelsList = new ArrayList();
    ...
    public QBloodOxygenLineChartView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public QBloodOxygenLineChartView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        this.linePaint.setShader(new LinearGradient(this.fingerX, this.lineHeight - this.bottomOffset, this.fingerX, 90.0f, this.colorsBoLine, (float[]) null, Shader.TileMode.CLAMP));
    ...
    public void setData(List<DataBean> data, boolean today) {
        ...
        for (DataBean dataBean : data) {
            ...
            if (dataBean.getMinValue() != 0 && dataBean.getMaxValue() != 0) {
                ...
                path.moveTo(((seconds * (i2 - (i3 * 2))) / CacheConstants.DAY) + i3, (this.lineHeight - this.bottomOffset) - (((dataBean.getMinValue() - 90) + 1) * i));
                Path path2 = this.path;
                int seconds2 = dataBean.getSeconds();
                int i4 = this.width;
                int i5 = this.leftRightOffset;
                path2.lineTo(((seconds2 * (i4 - (i5 * 2))) / CacheConstants.DAY) + i5, (this.lineHeight - this.bottomOffset) - (((dataBean.getMaxValue() - 90) + 1) * i));
            }
        }
        if (today) {
        ...
        this.fingerX = (((i6 - (i7 * 2)) * iIntValue) / CacheConstants.DAY) + i7;
        if (this.hashData.get(iIntValue + "") != null) {
            this.currBean = this.hashData.get(iIntValue + "");
        } else {
            if (today) {
                this.currBean = new DataBean(new DateUtil().getHour() * CacheConstants.HOUR, 0, 0);
            } else {
                this.currBean = new DataBean(43200, 0, 0);
            }
    ...
    private void doCalc() {
        ...
        if (onSelectedListener != null) {
            if (dataBean == null) {
                onSelectedListener.onSelected(new DataBean(iIntValue, 0, 0));
            } else {
