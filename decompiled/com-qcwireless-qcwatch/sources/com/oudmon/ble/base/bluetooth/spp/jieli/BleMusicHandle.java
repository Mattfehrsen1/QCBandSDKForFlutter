...
public class BleMusicHandle {
    ...
    private SerialListener serialListener = new SerialListener() { // from class: com.oudmon.ble.base.bluetooth.spp.jieli.BleMusicHandle.1
        ...
        public void onSerialConnect() {
            if (BleMusicHandle.this.outSerialListener != null) {
                BleMusicHandle.this.outSerialListener.onSerialConnect();
            }
        }
        ...
        public void onSerialConnectError(Exception exc) {
            if (BleMusicHandle.this.outSerialListener != null) {
                BleMusicHandle.this.outSerialListener.onSerialConnectError(exc);
            }
        }

        @Override // com.oudmon.ble.base.bluetooth.spp.SerialListener
        public void onSerialRead(byte[] bArr) throws InterruptedException, NumberFormatException {
            if (bArr != null) {
                XLog.i(ByteUtil.byteArrayToString(bArr));
                if ((bArr[0] & 255) == 188 && bArr[1] == 49) {
                    XLog.i("初始化完成，开始向手环发送实际文件");
                    BleMusicHandle.this.cmdSendPacket();
                    return;
                }
                if ((bArr[0] & 255) == 188 && bArr[1] == 50) {
                    if (BleMusicHandle.this.readNextBigPocket()) {
                        float f = Float.parseFloat(new DecimalFormat("#.00", new DecimalFormatSymbols(Locale.US)).format((((BleMusicHandle.this.mPocketIndex * 1024) * 1.0f) * 100.0f) / BleMusicHandle.this.totalSize));
                        XLog.i("向手环发送数据进度: " + f + ", 包序: " + ((int) BleMusicHandle.this.mPocketIndex) + "总包:" + BleMusicHandle.this.totalSize);
                        BleMusicHandle.this.mCallback.onProgress(Math.min(f, 100.0f));
                        return;
                    }
                    XLog.i("向手环发送数据完毕, 包序: " + ((int) BleMusicHandle.this.mPocketIndex));
                    BleMusicHandle.this.cmdCheck();
                    return;
                }
                if ((bArr[0] & 255) == 188 && bArr[1] == 51) {
                    XLog.i("===============回调 onComplete");
                    BleMusicHandle.this.mFileSend = new byte[0];
                    BleMusicHandle.this.mCallback.onComplete();
                    return;
                }
                if ((bArr[0] & 255) == 188 && bArr[1] == Byte.MIN_VALUE) {
                    ...
                    if (ByteUtil.bytesToInt(Arrays.copyOfRange(bArr, 8, 9)) == 0) {
                        BleMusicHandle.this.fileNames = new ArrayList();
                    }
                    XLog.i(Integer.valueOf(iBytesToInt));
                    if (iBytesToInt <= 0) {
                        BleMusicHandle.this.mCallback.onFileNames(BleMusicHandle.this.fileNames);
                        return;
                    } else {
                        BleMusicHandle.this.parseEbookData(bArr, 11, ByteUtil.bytesToInt(Arrays.copyOfRange(bArr, 10, 11)), iBytesToInt, 0);
                        return;
                    }
                }
                if ((bArr[0] & 255) == 188 && bArr[1] == -127) {
                    BleMusicHandle.this.mCallback.onDeleteSuccess(ByteUtil.bytesToInt(Arrays.copyOfRange(bArr, 6, 7)));
                }
            }
        }
        ...
        public void onSerialIoError(Exception exc) {
            if (BleMusicHandle.this.outSerialListener != null) {
                BleMusicHandle.this.outSerialListener.onSerialIoError(exc);
            }
        }
    };
    private IEbookCallback mCallback = new IEbookCallback() { // from class: com.oudmon.ble.base.bluetooth.spp.jieli.BleMusicHandle.2
        @Override // com.oudmon.ble.base.communication.file.IEbookCallback
        public void onFileNames(ArrayList<String> arrayList) {
            Iterator it = BleMusicHandle.this.mCallbackArray.iterator();
            while (it.hasNext()) {
    ...
    private OnGattEventCallback callback = new OnGattEventCallback() { // from class: com.oudmon.ble.base.bluetooth.spp.jieli.BleMusicHandle.3
        ...
        public void onReceivedData(String str, byte[] bArr) throws InterruptedException, NumberFormatException {
            if (bArr != null) {
                ...
                if ((bArr[0] & 255) == 188 && bArr[1] == 50) {
                    ...
                    BleMusicHandle.this.mCallback.onDeleteSuccess(iBytesToInt);
                    return;
                }
                if ((bArr[0] & 255) == 188 && bArr[1] == 51) {
                    XLog.i("===============回调 onComplete");
                ...
                if ((bArr[0] & 255) != 188 || bArr[1] != Byte.MIN_VALUE) {
                    if (BleMusicHandle.this.mReceiving) {
                        System.arraycopy(bArr, 0, BleMusicHandle.this.mReceivedData, BleMusicHandle.this.mReceivedCount, bArr.length);
                        BleMusicHandle.access$1112(BleMusicHandle.this, bArr.length);
                        BleMusicHandle bleMusicHandle = BleMusicHandle.this;
                        bleMusicHandle.mReceiving = bleMusicHandle.mReceivedCount < BleMusicHandle.this.mTotalCount;
                        XLog.i("文件：->3 mTotalCount: " + BleMusicHandle.this.mTotalCount + ", mReceivedCount: " + BleMusicHandle.this.mReceivedCount + ", mReceiving: " + BleMusicHandle.this.mReceiving);
                        if (BleMusicHandle.this.mReceiving) {
                            return;
                        }
                        XLog.i("文件：->4 ->" + DataTransferUtils.getHexString(BleMusicHandle.this.mReceivedData));
                        int iBytesToInt2 = ByteUtil.bytesToInt(Arrays.copyOfRange(BleMusicHandle.this.mReceivedData, 1, 2));
                        if (ByteUtil.bytesToInt(Arrays.copyOfRange(BleMusicHandle.this.mReceivedData, 2, 3)) == 0) {
                            BleMusicHandle.this.fileNames = new ArrayList();
                        }
                        XLog.i(Integer.valueOf(iBytesToInt2));
                        if (iBytesToInt2 > 0) {
                            int iBytesToInt3 = ByteUtil.bytesToInt(Arrays.copyOfRange(BleMusicHandle.this.mReceivedData, 4, 5));
                            BleMusicHandle bleMusicHandle2 = BleMusicHandle.this;
                            bleMusicHandle2.parseEbookData(bleMusicHandle2.mReceivedData, 5, iBytesToInt3, iBytesToInt2, 0);
                            return;
                        }
                        BleMusicHandle.this.mCallback.onFileNames(BleMusicHandle.this.fileNames);
                        return;
                ...
                BleMusicHandle.this.mTotalCount = DataTransferUtils.bytesToShort(bArr, 2);
                BleMusicHandle.this.mReceivedCount = bArr.length - 6;
                BleMusicHandle bleMusicHandle3 = BleMusicHandle.this;
                bleMusicHandle3.mReceivedData = new byte[bleMusicHandle3.mTotalCount];
                System.arraycopy(bArr, 6, BleMusicHandle.this.mReceivedData, 0, BleMusicHandle.this.mReceivedCount);
                BleMusicHandle bleMusicHandle4 = BleMusicHandle.this;
                bleMusicHandle4.mReceiving = bleMusicHandle4.mReceivedCount < BleMusicHandle.this.mTotalCount;
                XLog.i("文件：-> 1mTotalCount: " + BleMusicHandle.this.mTotalCount + ", mReceivedCount: " + BleMusicHandle.this.mReceivedCount + ", mReceiving: " + BleMusicHandle.this.mReceiving);
                if (BleMusicHandle.this.mReceiving) {
                    return;
                }
                XLog.i("文件：->2->" + DataTransferUtils.getHexString(BleMusicHandle.this.mReceivedData));
                int iBytesToInt4 = ByteUtil.bytesToInt(Arrays.copyOfRange(BleMusicHandle.this.mReceivedData, 1, 2));
                if (ByteUtil.bytesToInt(Arrays.copyOfRange(BleMusicHandle.this.mReceivedData, 2, 3)) == 0) {
                    BleMusicHandle.this.fileNames = new ArrayList();
                }
                XLog.i(Integer.valueOf(iBytesToInt4));
                if (iBytesToInt4 > 0) {
                    int iBytesToInt5 = ByteUtil.bytesToInt(Arrays.copyOfRange(BleMusicHandle.this.mReceivedData, 4, 5));
                    BleMusicHandle bleMusicHandle5 = BleMusicHandle.this;
                    bleMusicHandle5.parseEbookData(bleMusicHandle5.mReceivedData, 5, iBytesToInt5, iBytesToInt4, 0);
                    return;
                }
                BleMusicHandle.this.mCallback.onFileNames(BleMusicHandle.this.fileNames);
            }
    ...
    static /* synthetic */ int access$1112(BleMusicHandle bleMusicHandle, int i) {
    ...
    public void start(int i) throws InterruptedException {
        BleThreadManager.getInstance().addData(new BleDataBean(addHeader(128, new byte[]{1, (byte) i}), this.mPackageLength));
    }
    ...
    public void deleteMusic(int i, String str) {
        try {
            ...
            BleThreadManager.getInstance().addData(new BleDataBean(addHeader(129, ByteUtil.concat(new byte[]{1, (byte) i, (byte) bytes.length}, bytes)), this.mPackageLength));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public boolean executeFilePrepare(String str) {
        ...
        try {
            ...
            XLog.e("准备发送的文件.. dataSize = " + this.mFileSend.length + ", readSize = " + randomAccessFile2.read(bArr, 0, bArr.length));
            try {
