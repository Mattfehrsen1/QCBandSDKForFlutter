...
public class SppHandle {
    ...
    private int sizeA = 1;
    private int sizeB = 1;
    private SerialListener serialListener = new SerialListener() { // from class: com.oudmon.ble.base.bluetooth.spp.jieli.SppHandle.1
        ...
        public void onSerialConnect() {
            if (SppHandle.this.outSerialListener != null) {
                SppHandle.this.outSerialListener.onSerialConnect();
            }
        }
        ...
        public void onSerialConnectError(Exception exc) {
            if (SppHandle.this.outSerialListener != null) {
                SppHandle.this.outSerialListener.onSerialConnectError(exc);
            }
        }
        ...
        public void onSerialRead(byte[] bArr) throws NumberFormatException, IOException {
            if (bArr != null) {
                ...
                if ((bArr[0] & 255) == 188 && bArr[1] == 50) {
                    ...
                    if (iBytesToInt == 0) {
                        if (SppHandle.this.readNextBigPocket()) {
                            float f = Float.parseFloat(new DecimalFormat("#.00", new DecimalFormatSymbols(Locale.US)).format((((SppHandle.this.mPocketIndex * SppHandle.LongBagLength) * 1.0f) * 100.0f) / SppHandle.this.totalSize));
                            XLog.i("向手环发送数据进度: " + f + ", 包序: " + ((int) SppHandle.this.mPocketIndex) + "总包:" + SppHandle.this.totalSize);
                            SppHandle.this.mCallback.onProgress(Math.min(f, 100.0f));
                            return;
                        }
                        XLog.i("向手环发送数据完毕, 包序: " + ((int) SppHandle.this.mPocketIndex));
                        SppHandle.this.cmdCheck();
                        return;
                    }
                    SppHandle.this.mCallback.onDeleteSuccess(iBytesToInt);
                    return;
                }
                if ((bArr[0] & 255) == 188 && bArr[1] == 51) {
                    XLog.i("===============回调 onComplete");
                    SppHandle.this.mFileSend = new byte[0];
                    SppHandle.this.mCallback.onComplete();
                    return;
                }
                if ((bArr[0] & 255) != 188 || bArr[1] != Byte.MIN_VALUE) {
                    if (SppHandle.this.mReceiving) {
                        System.arraycopy(bArr, 0, SppHandle.this.mReceivedData, SppHandle.this.mReceivedCount, bArr.length);
                        SppHandle.access$812(SppHandle.this, bArr.length);
                        SppHandle sppHandle = SppHandle.this;
                        sppHandle.mReceiving = sppHandle.mReceivedCount < SppHandle.this.mTotalCount;
                        if (SppHandle.this.mReceiving) {
                            return;
                        }
                        int iBytesToInt2 = ByteUtil.bytesToInt(Arrays.copyOfRange(SppHandle.this.mReceivedData, 1, 2));
                        if (ByteUtil.bytesToInt(Arrays.copyOfRange(SppHandle.this.mReceivedData, 2, 3)) == 0) {
                            SppHandle.this.fileNames = new ArrayList();
                        }
                        XLog.i(Integer.valueOf(iBytesToInt2));
                        if (iBytesToInt2 > 0) {
                            int iBytesToInt3 = ByteUtil.bytesToInt(Arrays.copyOfRange(SppHandle.this.mReceivedData, 4, 5));
                            SppHandle sppHandle2 = SppHandle.this;
                            sppHandle2.parseEbookData(sppHandle2.mReceivedData, 5, iBytesToInt3, iBytesToInt2, 0);
                            return;
                        }
                        SppHandle.this.mCallback.onFileNames(SppHandle.this.fileNames);
                        return;
                    }
                    if ((bArr[0] & 255) == 188 && bArr[1] == -127) {
                        SppHandle.this.mCallback.onDeleteSuccess(ByteUtil.bytesToInt(Arrays.copyOfRange(bArr, 6, 7)));
                        return;
                    }
                    return;
                }
                SppHandle.this.mTotalCount = DataTransferUtils.bytesToShort(bArr, 2);
                SppHandle.this.mReceivedCount = bArr.length - 6;
                SppHandle sppHandle3 = SppHandle.this;
                sppHandle3.mReceivedData = new byte[sppHandle3.mTotalCount];
                System.arraycopy(bArr, 6, SppHandle.this.mReceivedData, 0, SppHandle.this.mReceivedCount);
                SppHandle sppHandle4 = SppHandle.this;
                sppHandle4.mReceiving = sppHandle4.mReceivedCount < SppHandle.this.mTotalCount;
                if (SppHandle.this.mReceiving) {
                    return;
                }
                int iBytesToInt4 = ByteUtil.bytesToInt(Arrays.copyOfRange(SppHandle.this.mReceivedData, 1, 2));
                if (ByteUtil.bytesToInt(Arrays.copyOfRange(SppHandle.this.mReceivedData, 2, 3)) == 0) {
                    SppHandle.this.fileNames = new ArrayList();
                }
                XLog.i(Integer.valueOf(iBytesToInt4));
                if (iBytesToInt4 > 0) {
                    int iBytesToInt5 = ByteUtil.bytesToInt(Arrays.copyOfRange(SppHandle.this.mReceivedData, 4, 5));
                    SppHandle sppHandle5 = SppHandle.this;
                    sppHandle5.parseEbookData(sppHandle5.mReceivedData, 5, iBytesToInt5, iBytesToInt4, 0);
                    return;
                }
                SppHandle.this.mCallback.onFileNames(SppHandle.this.fileNames);
            }
        ...
        public void onSerialIoError(Exception exc) {
            if (SppHandle.this.outSerialListener != null) {
                SppHandle.this.outSerialListener.onSerialIoError(exc);
            }
        }
    };
    private IEbookCallback mCallback = new IEbookCallback() { // from class: com.oudmon.ble.base.bluetooth.spp.jieli.SppHandle.2
        @Override // com.oudmon.ble.base.communication.file.IEbookCallback
        public void onFileNames(ArrayList<String> arrayList) {
            Iterator it = SppHandle.this.mCallbackArray.iterator();
            while (it.hasNext()) {
    ...
    private OnGattEventCallback callback = new OnGattEventCallback() { // from class: com.oudmon.ble.base.bluetooth.spp.jieli.SppHandle.3
            ...
            if (bArr != null) {
                ...
                if ((bArr[0] & 255) != 188 || bArr[1] != Byte.MIN_VALUE) {
                    if (SppHandle.this.mReceiving) {
                        ...
                        SppHandle sppHandle = SppHandle.this;
                        sppHandle.mReceiving = sppHandle.mReceivedCount < SppHandle.this.mTotalCount;
                        XLog.i("文件：->3 mTotalCount: " + SppHandle.this.mTotalCount + ", mReceivedCount: " + SppHandle.this.mReceivedCount + ", mReceiving: " + SppHandle.this.mReceiving);
                        if (SppHandle.this.mReceiving) {
                            return;
                        }
                        XLog.i("文件：->4 ->" + DataTransferUtils.getHexString(SppHandle.this.mReceivedData));
                        int iBytesToInt2 = ByteUtil.bytesToInt(Arrays.copyOfRange(SppHandle.this.mReceivedData, 1, 2));
                        if (ByteUtil.bytesToInt(Arrays.copyOfRange(SppHandle.this.mReceivedData, 2, 3)) == 0) {
                            SppHandle.this.fileNames = new ArrayList();
                        }
                ...
                SppHandle sppHandle4 = SppHandle.this;
                sppHandle4.mReceiving = sppHandle4.mReceivedCount < SppHandle.this.mTotalCount;
                XLog.i("文件：-> 1mTotalCount: " + SppHandle.this.mTotalCount + ", mReceivedCount: " + SppHandle.this.mReceivedCount + ", mReceiving: " + SppHandle.this.mReceiving);
                if (SppHandle.this.mReceiving) {
                    return;
                }
                XLog.i("文件：->2->" + DataTransferUtils.getHexString(SppHandle.this.mReceivedData));
                int iBytesToInt4 = ByteUtil.bytesToInt(Arrays.copyOfRange(SppHandle.this.mReceivedData, 1, 2));
                if (ByteUtil.bytesToInt(Arrays.copyOfRange(SppHandle.this.mReceivedData, 2, 3)) == 0) {
                    SppHandle.this.fileNames = new ArrayList();
                }
    ...
    static /* synthetic */ int access$812(SppHandle sppHandle, int i) {
        int i2 = sppHandle.mReceivedCount + i;
        sppHandle.mReceivedCount = i2;
        return i2;
    }
    ...
    */
    public boolean executeFilePrepare(java.lang.String r13) throws java.lang.Throwable {
        ...
        */
        throw new UnsupportedOperationException("Method not decompiled: com.oudmon.ble.base.bluetooth.spp.jieli.SppHandle.executeFilePrepare(java.lang.String):boolean");
    }
    ...
    */
    public static java.lang.String charset(java.lang.String r8) throws java.io.IOException {
        ...
        L99:
            return r0
        */
        throw new UnsupportedOperationException("Method not decompiled: com.oudmon.ble.base.bluetooth.spp.jieli.SppHandle.charset(java.lang.String):java.lang.String");
    }
        ...
        try {
            ...
            if (i2 < i3) {
                if (this.mFileSend.length > 0) {
                    int iMin = Math.min(i, i3 - (s * i));
                    byte[] bArr = new byte[iMin];
                    System.arraycopy(this.mFileSend, this.mPocketIndex * LongBagLength, bArr, 0, iMin);
                ...
                int i5 = this.sizeA;
                if (i4 < i5) {
                    int iMin2 = Math.min(1024, i5 - (s2 * 1024));
