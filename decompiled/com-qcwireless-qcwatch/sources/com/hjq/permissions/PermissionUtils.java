...
final class PermissionUtils {
    ...
    static void optimizePermissionResults(Activity activity, String[] strArr, int[] iArr) {
        for (int i = 0; i < strArr.length; i++) {
            ...
            if (AndroidVersion.isAndroid13() && AndroidVersion.getTargetSdkVersionCode(activity) >= 33 && equalsPermission(str, "android.permission.WRITE_EXTERNAL_STORAGE")) {
    ...
    static boolean areActivityIntent(Context context, Intent intent) {
        ...
        if (AndroidVersion.isAndroid13()) {
            return !packageManager.queryIntentActivities(intent, PackageManager.ResolveInfoFlags.of(PlaybackStateCompat.ACTION_PREPARE_FROM_SEARCH)).isEmpty();
        }
    ...
    static Intent getSmartPermissionIntent(Context context, List<String> list) {
        ...
        } else if (size == 3 && AndroidVersion.isAndroid11() && containsPermission(list, Permission.MANAGE_EXTERNAL_STORAGE) && containsPermission(list, "android.permission.READ_EXTERNAL_STORAGE") && containsPermission(list, "android.permission.WRITE_EXTERNAL_STORAGE")) {
