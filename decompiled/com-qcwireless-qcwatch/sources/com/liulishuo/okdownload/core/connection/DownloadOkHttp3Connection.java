...
public class DownloadOkHttp3Connection implements DownloadConnection, DownloadConnection.Connected {
    ...
    public DownloadConnection.Connected execute() throws IOException {
        Request requestBuild = this.requestBuilder.build();
        this.request = requestBuild;
        this.response = this.client.newCall(requestBuild).execute();
        return this;
    }
    ...
    public String getRedirectLocation() {
        Response responsePriorResponse = this.response.priorResponse();
        if (responsePriorResponse != null && this.response.isSuccessful() && RedirectUtil.isRedirect(responsePriorResponse.code())) {
