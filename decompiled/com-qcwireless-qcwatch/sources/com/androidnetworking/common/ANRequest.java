        ...
        try {
            for (Map.Entry<String, MultipartStringBody> entry : this.mMultiPartParameterMap.entrySet()) {
                ...
                if (value.contentType != null) {
                    mediaType3 = MediaType.parse(value.contentType);
                }
                type.addPart(Headers.of(Util.CONTENT_DISPOSITION, "form-data; name=\"" + entry.getKey() + "\""), RequestBody.create(mediaType3, value.value));
            }
            for (Map.Entry<String, List<MultipartFileBody>> entry2 : this.mMultiPartFileMap.entrySet()) {
                for (MultipartFileBody multipartFileBody : entry2.getValue()) {
                    ...
                    type.addPart(Headers.of(Util.CONTENT_DISPOSITION, "form-data; name=\"" + entry2.getKey() + "\"; filename=\"" + name + "\""), RequestBody.create(mediaType, multipartFileBody.file));
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
