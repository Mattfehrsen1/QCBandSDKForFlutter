...
public final class BridgeInterceptor implements Interceptor {
    ...
    public Response intercept(Interceptor.Chain chain) throws IOException {
        ...
        if (requestBodyBody != null) {
            ...
            if (jContentLength != -1) {
                builderNewBuilder.header(Util.CONTENT_LENGTH, String.valueOf(jContentLength));
                builderNewBuilder.removeHeader(Util.TRANSFER_ENCODING);
            } else {
                builderNewBuilder.header(Util.TRANSFER_ENCODING, "chunked");
                builderNewBuilder.removeHeader(Util.CONTENT_LENGTH);
            }
        }
        boolean z = false;
        if (request.header("Host") == null) {
            builderNewBuilder.header("Host", okhttp3.internal.Util.toHostHeader$default(request.url(), false, 1, null));
        }
        if (request.header("Connection") == null) {
            builderNewBuilder.header("Connection", "Keep-Alive");
        }
        if (request.header("Accept-Encoding") == null && request.header(Util.RANGE) == null) {
        ...
        if (request.header("User-Agent") == null) {
            builderNewBuilder.header("User-Agent", okhttp3.internal.Util.userAgent);
        }
        ...
        if (z && StringsKt.equals("gzip", Response.header$default(responseProceed, "Content-Encoding", null, 2, null), true) && HttpHeaders.promisesBody(responseProceed) && (responseBodyBody = responseProceed.body()) != null) {
            ...
            builderRequest.headers(responseProceed.headers().newBuilder().removeAll("Content-Encoding").removeAll(Util.CONTENT_LENGTH).build());
