...
final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cont) {
        Object objM2591constructorimpl;
        boolean z;
            ...
            while (true) {
                if (segment2.getId() < j || segment2.getRemoved()) {
                    ...
                    if (obj == ConcurrentLinkedListKt.CLOSED) {
                        objM2591constructorimpl = SegmentOrClosed.m2591constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    objM2591constructorimpl = SegmentOrClosed.m2591constructorimpl(segment2);
                    break;
                }
            }
            if (SegmentOrClosed.m2596isClosedimpl(objM2591constructorimpl)) {
                break;
            }
            Segment segmentM2594getSegmentimpl = SegmentOrClosed.m2594getSegmentimpl(objM2591constructorimpl);
            while (true) {
                Segment segment3 = (Segment) this.tail;
                if (segment3.getId() >= segmentM2594getSegmentimpl.getId()) {
                    break;
                }
                if (!segmentM2594getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                    z = false;
                    break;
                }
                if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(tail$FU, this, segment3, segmentM2594getSegmentimpl)) {
                    if (segment3.decPointers$kotlinx_coroutines_core()) {
                        segment3.remove();
                    }
                } else if (segmentM2594getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                    segmentM2594getSegmentimpl.remove();
                }
            }
            z = true;
        } while (!z);
        SemaphoreSegment semaphoreSegment = (SemaphoreSegment) SegmentOrClosed.m2594getSegmentimpl(objM2591constructorimpl);
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        do {
            ...
            while (true) {
                ...
                if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(head$FU, this, segment3, segmentM2594getSegmentimpl)) {
                    if (segment3.decPointers$kotlinx_coroutines_core()) {
                        segment3.remove();
                    }
                } else if (segmentM2594getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
