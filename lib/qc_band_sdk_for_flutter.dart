...
import 'utils/real_time_sport_parser.dart';
...
// Byte to Int Util class
  ...
  static Map DataParsingWithData(List<int> value) {
    int commandId = value[1]; // Extract command ID for logging
    switch (commandId) {
      ...
      case QcBandSdkConst.sportMode:
        return {"status": "success", "message": "Sport mode command sent"};
      
      // Real-time sport data (live workout metrics)
      case QcBandSdkConst.realTimeSportData:
      case QcBandSdkConst.sportDataNotify:
        // Check if this looks like real-time data
        if (RealTimeSportParser.isRealTimeSportPacket(value)) {
          print('🏃 Processing real-time sport data packet (cmd: $commandId)');
          return RealTimeSportParser.parseRealTimePacket(value);
        } else {
          print('⚠️ Packet doesn\'t match real-time sport format, falling back to historical parser');
          return ResolveUtil.getSportMode(value);
        }
        
      // Historical sport data (summaries)
      case 18: // Sport Data from band
      case QcBandSdkConst.getSportMode:
        print('📊 Processing historical sport data (cmd: $commandId)');
        return ResolveUtil.getSportMode(value);
        
      case QcBandSdkConst.getStressData:
        return ResolveUtil.getStressData(value);
  ...
  static Uint8List sportMode(int sportType, int status) {
    List<int> value = _generateInitValue();
    value[0] = QcBandSdkConst.startSportMode;
    value[1] = sportType;
    value[2] = status;
    _crcValue(value);
...
// to 7 days
  ...
  static Uint8List getStressData(int offset) {
    final List<int> value = _generateInitValue();
    value[0] = QcBandSdkConst.getStressData;
    value[1] = offset;
    _crcValue(value);
    return Uint8List.fromList(value);
  }
