...
// --- The highly specific parseHRVData function ---
  ...
  sleepDetailData() async {
    ...
    List<int> realDeviceData = [
      188, 39, 45, 0, 229, 149, 1, 3, 42, 111, 5, 197, 1, 2, 18, 3, 38, 4, 18, 2, 24, 3, 13, 4, 15, 2, 32, 4, 17, 3, 18, 2, 31, 3, 36, 4, 21, 3, 21, 2, 24, 3, 34, 4, 15, 2, 56, 3, 56, 2, 15
    ];
    ...
    print("Manual decode - Start: $startMinutes minutes (${(startMinutes ~/ 60).toString().padLeft(2, '0')}:${(startMinutes % 60).toString().padLeft(2, '0')})");
    print("Manual decode - End: $endMinutes minutes (${(endMinutes ~/ 60).toString().padLeft(2, '0')}:${(endMinutes % 60).toString().padLeft(2, '0')})");
    ...
    if (realSummary.bedTime != null && realSummary.wakeTime != null) {
      print("  📍 Bed Time (local): ${realSummary.bedTime!.hour.toString().padLeft(2, '0')}:${realSummary.bedTime!.minute.toString().padLeft(2, '0')}");
      print("  📍 Wake Time (local): ${realSummary.wakeTime!.hour.toString().padLeft(2, '0')}:${realSummary.wakeTime!.minute.toString().padLeft(2, '0')}");
    }
    print("  Segments: ${realSummary.segments.length}");
        ...
        bool _validateLargeDataPacket(List<int> data, int expectedDay) {
          ...
          if (data.length != expectedActualLength && data.length != declaredLength) {
          ...
          if (data.length == 59 && totalDays == 1) {
            print("✅ VALIDATION: 59-byte packet detected as Standard Protocol (single day, ${data.length} bytes)");
  ...
  Future<List<int>> fetchSingleDayResponse(int day) async {
    ...
    int expectedDataLength = 0;
          ...
          print("🔍 _secondbluetoothCharacteristicNotification exists: ${_secondbluetoothCharacteristicNotification != null}");
          if (_secondbluetoothCharacteristicNotification != null) {
            print("🔍 Characteristic UUID: ${_secondbluetoothCharacteristicNotification!.uuid}");
          } else {
          ...
          _secondbluetoothCharacteristicNotification.value.listen((value) {
            ...
            if (value.isNotEmpty && value[1] == QcBandSdkConst.getSleepData) {
              ...
              if (value.length >= 6 && (value[0] & 255) == 188) {
                ...
                if (value.length - 6 >= expectedDataLength) {
                  ...
                  List<int> completePacket = List<int>.from(value);
                                      ...
                                      if (isValidMultiDayPacket(completePacket, day) && containsExpectedDay(completePacket, day)) {
                ...
                print("🔄 MULTI-PACKET START: Storing first chunk (${value.length} bytes)");
              ...
              if (tempPacketData != null) {
                print("🔄 CONTINUATION PACKET: Adding ${value.length} bytes to accumulated data");
                tempPacketData!.addAll(value);
                print("📊 ACCUMULATED: ${tempPacketData!.length} bytes, Expected: ${expectedDataLength + 6}");
                
                if (tempPacketData!.length - 6 == expectedDataLength) {
                  // Complete assembly: Process accumulated data
                  print("✅ ASSEMBLY COMPLETE: Processing ${tempPacketData!.length} bytes");
                  List<int> assembledPacket = List<int>.from(tempPacketData!);
                  
                  if (isValidMultiDayPacket(assembledPacket, day) && containsExpectedDay(assembledPacket, day)) {
                    print("✅ ASSEMBLED PACKET ACCEPTED: Valid multi-packet for day $day");
                  ...
                  print("❌ ASSEMBLY ERROR: Too much data received (${tempPacketData!.length - 6} > $expectedDataLength)");
                  tempPacketData = null;
                  expectedDataLength = 0;
                }
    ...
    if (todayData.isNotEmpty && todayData.length >= 13) {
      ...
      if (enhancedSummary.segments.isNotEmpty) {
        ...
        for (int i = 0; i < 5 && i < enhancedSummary.segments.length; i++) {
          ...
          print("    ${seg.segmentStart} - ${seg.segmentEnd} (stage: ${seg.stageType}, quality: ${seg.originalQuality})");
        }
      }
    ...
    if (day1Data.isNotEmpty && day1Data.length >= 13) {
      ...
      if (day1Summary.bedTime != null && day1Summary.wakeTime != null) {
        print("  📍 Bed Time (local): ${day1Summary.bedTime!.hour.toString().padLeft(2, '0')}:${day1Summary.bedTime!.minute.toString().padLeft(2, '0')}");
        print("  📍 Wake Time (local): ${day1Summary.wakeTime!.hour.toString().padLeft(2, '0')}:${day1Summary.wakeTime!.minute.toString().padLeft(2, '0')}");
      }
    ...
    for (int i = 1; i < 7; i++) {
      ...
      if (combinedResponse.isNotEmpty && combinedResponse.length >= 13) {
        ...
        Map<String, dynamic> sleepDataJson = {
          "day_$i": {
            ...
            "stage_breakdown": {
              ...
              "rem_sleep_minutes": enhancedSummary.durations['rapidEyeMovement'],
              "awake_minutes": enhancedSummary.durations['awake'],
            },
            "segments": enhancedSummary.segments.map((seg) => {
        ...
        print("\nSleep Summary - Day $i:");
        print("  ${enhancedSummary.bedTime?.hour.toString().padLeft(2, '0')}:${enhancedSummary.bedTime?.minute.toString().padLeft(2, '0')} → ${enhancedSummary.wakeTime?.hour.toString().padLeft(2, '0')}:${enhancedSummary.wakeTime?.minute.toString().padLeft(2, '0')} (${enhancedSummary.durations['totalDuration']} min)");
        print("  Deep: ${enhancedSummary.durations['deepSleep']}min | Light: ${enhancedSummary.durations['lightSleep']}min | REM: ${enhancedSummary.durations['rapidEyeMovement']}min | Awake: ${enhancedSummary.durations['awake']}min");
        print("  ${enhancedSummary.segments.length} segments");
        
        // Show segment details
        print("\nSegment Timeline (${enhancedSummary.segments.length} total):");
        for (int segIdx = 0; segIdx < enhancedSummary.segments.length && segIdx < 10; segIdx++) {
          final seg = enhancedSummary.segments[segIdx];
          final startTime = "${seg.segmentStart.hour.toString().padLeft(2, '0')}:${seg.segmentStart.minute.toString().padLeft(2, '0')}";
          final endTime = "${seg.segmentEnd.hour.toString().padLeft(2, '0')}:${seg.segmentEnd.minute.toString().padLeft(2, '0')}";
          final duration = seg.segmentEnd.difference(seg.segmentStart).inMinutes;
          final stageName = _getStageDisplayName(seg.stageType);
          print("  ${segIdx + 1}. $startTime → $endTime ($duration min) - $stageName");
        }
        if (enhancedSummary.segments.length > 10) {
          print("  ... and ${enhancedSummary.segments.length - 10} more segments");
        }
