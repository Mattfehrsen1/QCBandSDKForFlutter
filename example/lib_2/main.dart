...
class _HomePageState extends State<HomePage> {
  StreamSubscription<List<ScanResult>>?
      _scanResultSubs; // device scan subscription
  BluetoothDevice? _device;
  BluetoothCharacteristic? _charWrite; // custom write
  BluetoothCharacteristic? _charNotify; // custom notify
  StreamSubscription<List<int>>?
      _charNotifySubs; // custom notify stream subscription
  StreamSubscription<BluetoothConnectionState>?
      _connStateSubs; // device connection state subscription
  ...
  Future<void> _connect() async {
    ...
    if (_device != null) {
      _device!
          .cancelWhenDisconnected(_connStateSubs!, delayed: true, next: true);
          ...
          try {
            ...
            _connStateSubs = _device!.connectionState
              ...
              if (state == BluetoothConnectionState.disconnected) {
                ...
                if (reason != null) {
                  debugPrint(
                      'device disconnection reason: ${_device!.disconnectReason}');
                  if (reason.platform == ErrorPlatform.android &&
                      reason.code == 133) {
                    _error =
                        'ANDROID_SPECIFIC_ERROR occurred. Multiple attempts to reconnect (3+) usually solve it.';
                  }
                }
              }
            ...
            _device!.cancelWhenDisconnected(_connStateSubs!,
        ...
        if (service.uuid.str128 == ring.Uuid.cmdService.str128) {
          for (BluetoothCharacteristic c in service.characteristics) {
            ...
            // Find the Char for receiving 16-byte notifications from the ring and subscribe
              ...
              await _charNotifySubs?.cancel();
              _charNotifySubs =
                  _charNotify!.onValueReceived.listen(_onNotificationData);
    ...
    if (data[0] == ring.Notification.battery.code) {
      ...
      if (data[1] == ring.GeneralSubtype.stepsCaloriesDistance.code) {
        var (steps, calories, distance) =
            ring.parseNotifStepsCaloriesDistanceData(data);
        _steps = steps;
        _calories = calories;
        _distance = distance;
      ...
      if (data[1] == ring.RawSensorSubtype.accelerometer.code) {
        ...
        var netGforce =
            (sqrt(rawX * rawX + rawY * rawY + rawZ * rawZ) / 512 - 1.0).abs();
  ...
  Future<void> _toggleAllRawData() async {
    await _sendCommand(_rawDataOn
        ? ring.Command.disableAllRawData.bytes
        : ring.Command.enableAllRawData.bytes);
    _rawDataOn = !_rawDataOn;
  }

  Future<void> _toggleWaveGesture() async {
    await _sendCommand(_waveGestureOn
        ? ring.Command.disableWaveGesture.bytes
        : ring.Command.enableWaveGesture.bytes);
      ...
      body: Align(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            Text(
              'Sent Command: ${_sentData ?? 'No Data'}',
            ),
            const SizedBox(height: 10),
            Text(
              'Response: ${_receivedData ?? 'No Data'}',
            ),
            const SizedBox(height: 10),
            Text(
              'Error: ${_error ?? 'None'}',
            ...
            const Divider(),
            ElevatedButton(
              onPressed: (_device == null || _device!.isDisconnected)
                  ? _connect
                  : _disconnect,
              child: Text((_device == null || _device!.isDisconnected)
                  ? "Connect"
                  : "Disconnect"),
            ),
            const Divider(),
            ...
            SizedBox(
                height: 60,
                child:
                    Wrap(direction: Axis.vertical, runSpacing: 30.0, children: [
                  Text(
                    'Battery Level: ${_batteryLevel != null ? '$_batteryLevel%' : ''}',
                  ),
                  const SizedBox(height: 10),
                  Text(
                    'Is Charging: ${_batteryIsCharging ?? ''}',
                  ),
                  const SizedBox(height: 10),
                  Text(
                    'Steps: ${_steps ?? ''}',
                  ...
                  const SizedBox(height: 10),
                  Text(
                    'SpO2: ${_spO2Percentage != null ? '$_spO2Percentage%' : ''}',
                  ),
                  const SizedBox(height: 10),
                  Text(
                    'Stress: ${_stress ?? ''}',
            ...
            SizedBox(
                ...
                child:
                    Wrap(direction: Axis.vertical, runSpacing: 30.0, children: [
                  Text(
                    'SpO2 Raw: [${_spO2raw ?? ''}, ${_spO2a ?? ''}, ${_spO2b ?? ''}, ${_spO2c ?? ''}]',
                  ),
                  Text(
                    'PPG Raw: [${_ppgRaw ?? ''}, ${_ppgMax ?? ''}, ${_ppgMin ?? ''}, ${_ppgDiff ?? ''}]',
                  ),
                ])),
            const Divider(),
            ...
            SizedBox(
              ...
              child:
                ...
                Column(children: [
                  Row(children: [
                    Text(
                      'Scroll: ${_scroll?.toStringAsFixed(2) ?? ''}',
                    ),
                    Slider(
                      value: _scroll ?? 0,
                      min: -pi,
                  ...
                  Row(children: [
                    Text(
                      'Impact: ${_impact?.toStringAsFixed(2) ?? ''}',
                    ),
                    Slider(
                      value: _impact?.clamp(0, 1) ?? 0,
                      min: 0,
                      max: 1,
                      onChanged: (val) => {},
                    ),
