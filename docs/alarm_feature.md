...
Add a weekday alarm at 07:00 named "Work":
...
final pkt = QCBandSDK.buildVendorAlarmWriteMany([
  {
    ...
    'enabled': true,
    // Su..Sa
    'repeatDays': const [false, true, true, true, true, true, false],
    'content': 'Work',
  },
]);
await _secondbluetoothCharacteristicWrite.write(pkt);
...
Add a weekend alarm at 09:30 named "Weekend":
...
final pkt = QCBandSDK.buildVendorAlarmWriteMany([
  {
    ...
    'enabled': true,
    'repeatDays': const [true, false, false, false, false, false, true],
    'content': 'Weekend',
  },
]);
await _secondbluetoothCharacteristicWrite.write(pkt);
...
Disable (turn off) an existing alarm without deleting: re-send full list with `enabled: false` for that item.

```dart
// Read
await _secondbluetoothCharacteristicWrite.write(QCBandSDK.buildVendorAlarmRead());
// In notify handler, parse payload → alarms
final alarms = QCBandSDK.parseVendorAlarmPayload(payload);

// Example: disable first alarm, keep repeat/time the same
if (alarms.isNotEmpty) {
  final first = alarms.first;
  final minutes = (first['minutes'] as int);
  final days = (first['repeatDays'] as List<bool>);
  final name = (first['content'] as String? ?? '');
  final newPkt = QCBandSDK.buildVendorAlarmWriteMany([
    {
  ...
  await _secondbluetoothCharacteristicWrite.write(newPkt);
}
```

Update an alarm’s time and name (e.g., move to 06:45, rename to "Gym"):

```dart
final updated = QCBandSDK.buildVendorAlarmWriteMany([
  {
    'minutesSinceMidnight': 6 * 60 + 45,
    'enabled': true,
    'repeatDays': const [false, true, true, true, true, true, false],
    'content': 'Gym',
  },
]);
await _secondbluetoothCharacteristicWrite.write(updated);
```

Clear all vendor alarms:

```dart
await _secondbluetoothCharacteristicWrite.write(QCBandSDK.buildVendorAlarmClear());
```

Quick two-minute test alarm with time sync and DND check (condensed):

```dart
// 1) DND read (optional diagnostic)
await _secondbluetoothCharacteristicWrite.write(ResolveUtil().addHeader(0x06, Uint8List.fromList([0x01])));

// 2) Set device time (UART)
await _bluetoothCharacteristicWrite.write(QCBandSDK.setDeviceTime(0));

// 3) Write alarm +2 minutes from now (vendor)
final now = DateTime.now().add(const Duration(minutes: 2));
final minutes = now.hour * 60 + now.minute;
await _secondbluetoothCharacteristicWrite.write(
  QCBandSDK.buildVendorAlarmWriteSingle(
    minutes,
    enabled: true,
    repeatDays: const [true, true, true, true, true, true, true],
    content: 'Alarm',
